            ...
            if(pram.issuer === pram.subject){
                ...
                const ocsp = ocsps.filter(v=>{
                    try{
                        ...
                        const signHex = (responseData as any).tohex();
...
class DSS{
        ...
        for(const cert of allCert){
            ...
            const certId:CertId = (()=>{
                try{
                    ...
                    throw new Error('Unsuppxort cert file.')
                }
            })();
            if(!certs.find(v=>{
    ...
    private _getDSS = async (subjectCertX509:X509) => {
        ...
        for(const cdp of cdpList?.array||[]){
            ...
            const uriList:Array<string> = cdp.dpname.full.map((v:any)=>v.uri);
            ...
            for(let i=0;i<AIAInfo.ocsp.length;i++){
                ...
                if(ocspUrl){
                    try{
                        ...
                        const ocspRequest = (KJUR.asn1.ocsp as any).OCSPUtil.getRequestHex(certHex2PEM(issuerX509.hex),certHex2PEM(subjectCertX509.hex));
...
export class SignTool{
    ...
    static getSignedHex = async (
        ...
        const keyObj = KEYUTIL.getKey(buffer2Hex(signer.key),signer.passphrase,"pkcs8prv")  as any;
        ...
        if(options?.tsa){
            ...
            const signedData = new (KJUR.asn1.cms as any).SignedData(params) as any;
            const cmsParser = new KJUR.asn1.cms.CMSParser();
            ...
            const sidnedData = new (KJUR.asn1.cms as any).SignedData(params);
    ...
    public static getTsaSigneture = async (signTrg:Uint8Array,tsaHashAlg:SupportHashAlgorithm,tsaServer:{url:string}):Promise<string> => {
        try{
            ...
            const tsaReqData = new KJUR.asn1.tsp.TimeStampReq(tsreqPram as any);
            //TSA Request
            ...
            if(tstRes.statusinfo === "granted"|| (typeof tstRes.statusinfo !=="string" && tstRes.statusinfo?.status === "granted")){
                ...
                if(reqNonce.toLowerCase() === resNonce.toLowerCase()){
                    const tstInfoObj = ASN1HEX.parse(buffer2Hex(tsaReq.data)) as any;
                    if(tstInfoObj.seq){
    ...
    public static chekCertUsage = (cert:Uint8Array) => {
        try{
            ...
            console.log(e)
            throw new Error("Unsupport ycert file.");
        }
